...
def main(bicycleinit: Connection, name: str, args: dict):
    ...
    sensor.write_header(['accel_x', 'accel_y', 'accel_z', 'pitch', 'roll'])
    ...
    try:
        while True:
            ...
            accel_x_f, accel_y_f, accel_z_f = float(accel_x), float(accel_y), float(accel_z)

            # Calculate Pitch and Roll
            pitch_rad = math.atan2(-accel_x_f, math.sqrt(accel_y_f**2 + accel_z_f**2))
            pitch = pitch_rad * RAD_TO_DEG
            
            roll_rad = math.atan2(accel_y_f, accel_z_f)
            roll = roll_rad * RAD_TO_DEG
            
            # Send Data to logger (at roughly 20Hz)
            sensor.write_measurement([accel_x_f, accel_y_f, accel_z_f, pitch, roll])
            time.sleep(0.05)
